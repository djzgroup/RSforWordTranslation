...
def get_word_translation_accuracy(lang1, word2id1, emb1, lang2, word2id2, emb2, dico_eval, exp_path, alpha, inv_K):
    """
    ...
    query = emb1[dico[:, 0]]
    inv_src_emb = get_inv_src(emb1, emb2, exp_path, inv_K)
    query = query.cpu()
    scores = alpha * query.mm(inv_src_emb.cpu().transpose(0, 1))
    scores.add_((1 - alpha) * query.mm(emb2.cpu().transpose(0, 1)))
    ...
    for k in [1, 5, 10]:
        ...
        logger.info("%i source words - %s - Precision at k = %i: %f" %
                    (len(matching), 'rs', k, precision_at_k))

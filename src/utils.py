...
MAIN_DUMP_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))
...
def get_inv_src(emb1, emb2, exp_path,  inv_K):
    inv_index = []
    n_tgt = emb2.size(0)
    bs = 128
    # n_src = emb1.size(0)
    ...
    # 如果没有生成则生成它
    path = os.path.join(exp_path, 'inv_src_emb0.pth')
    if os.path.isfile(path):
        for k in range(inv_K):
            path = os.path.join(exp_path, 'inv_src_emb' + str(k) + '.pth')
            inv_src['inv_src_emb' + str(k)] = torch.load(path)
    else:
        for i in range(0, n_tgt, bs):
            # compute source words scores add by linxi
            scores = emb2[i:min(n_tgt, i + bs)].mm(emb1.transpose(0, 1))
            _, index = scores.topk(inv_K, dim=1, largest=True, sorted=True)
            inv_index.append(index)
        inv_index = torch.cat(inv_index, 0)
        for k in range(inv_K):
            inv_src['inv_src_emb' + str(k)] = emb1[inv_index[:, k]]
            path = os.path.join(exp_path, 'inv_src_emb' + str(k) + '.pth')
            torch.save(inv_src['inv_src_emb' + str(k)], path)

    inv_src_emb = inv_src['inv_src_emb0']
    if inv_K != 1:
        de = 0
        for k in range(1, inv_K):
            inv_src_emb = inv_src_emb + (2 ** (inv_K - 1 - k)) * inv_src['inv_src_emb' + str(k)]
            if k == (inv_K - 1):
                inv_src_emb = inv_src_emb + ((2 ** (inv_K - 1) - 1) * inv_src['inv_src_emb0'])
                for kk in range(inv_K):
                    de = de + (2 ** k)
                inv_src_emb = inv_src_emb.div(de)
    return inv_src_emb
